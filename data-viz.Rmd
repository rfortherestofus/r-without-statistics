...
With that said, let's look at a few of the small polishes that Cédric and Georgios make. The first is to apply a theme. They use `theme_light()`, which makes the 
...
In addition to using `geom_col()` to make a stacked bar chart of the drought data, Cédric and Georgios also use `geom_rect()`. Let's add that and see what the plot looks like. 
...
The difference is subtle, but if you look closely you can see a gray background added behind the multicolor bars. This will become more important later on. 

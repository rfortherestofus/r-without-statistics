...
```{r gapminder-points, echo = TRUE, include = FALSE}
ggplot(
  data = gapminder_10_rows,
  mapping = aes(
    x = year,
...
```{r ref.label = "gapminder-points", echo = FALSE, include = TRUE, fig.cap="The same chart but with points added"}
```
...
```{r gapminder-line, echo = TRUE, include = FALSE}
ggplot(
  data = gapminder_10_rows,
  mapping = aes(
    x = year,
...
```{r ref.label = "gapminder-line", echo = FALSE, fig.cap="The data as a line chart"}
```
...
```{r gapminder-points-line, echo = TRUE, include = FALSE}
ggplot(
  data = gapminder_10_rows,
  mapping = aes(
    x = year,
...
```{r ref.label = "gapminder-points-line", echo = FALSE, fig.cap="The data with points and a line"}
```
...
```{r gapminder-bar, echo = TRUE, include = FALSE}
ggplot(
  data = gapminder_10_rows,
  mapping = aes(
    x = year,
...
```{r ref.label = "gapminder-bar", echo = FALSE, fig.cap="The data as a bar chart"}
```
...
```{r gapminder-bar-colors, echo = TRUE, include = FALSE}
ggplot(
  data = gapminder_10_rows,
  mapping = aes(
    x = year,
...
```{r ref.label = "gapminder-bar-colors", echo = FALSE, fig.cap="The same chart, now with added colors"}
```
...
```{r gapminder-viridis, echo = TRUE, include = FALSE}
ggplot(
  data = gapminder_10_rows,
  mapping = aes(
    x = year,
...
```{r ref.label = "gapminder-viridis", echo = FALSE, fig.cap="The same chart with a colorblind-friendly palette"}
```
...
```{r gapminder-theme, echo = TRUE, include = FALSE}
ggplot(
  data = gapminder_10_rows,
  mapping = aes(
    x = year,
...
```{r ref.label = "gapminder-theme", echo = FALSE, fig.cap="The same chart with `theme_minimal()` added"}
```
...
```{r southwest-2003-no-style, echo = TRUE, include = FALSE}
ggplot(
  data = southwest_2003,
  aes(
    x = week,
...
In the `ggplot()` function, we tell R to put `week` on the x axis and `percentage` on the y axis. We also use the `category` variable for our `fill` color. We then use `geom_col()` to create a bar chart in which the fill color of each bar represents the percentage of the region in a single week at each drought level. You can see the result in in \@ref(fig:southwest-2003-no-style).

```{r results='asis'}
print_nostarch_file_name()
```

```{r ref.label = "southwest-2003-no-style", echo = FALSE, fig.cap="One year and region of the drought visualization"}
```
...
```{r southwest-2003-with-color, echo = TRUE, include = FALSE}
ggplot(
  data = southwest_2003,
  aes(
    x = week,
...
```{r southwest-2003-xy-scales, echo = TRUE, include = FALSE}
ggplot(
  data = southwest_2003,
  aes(
    x = week,
...
```{r ref.label = "southwest-2003-xy-scales", echo = FALSE, fig.cap="One year and region of the drought visualization with adjustments to the x and y axes"}
```
...
```{r drought-viz-faceted, fig.height = 8, echo = TRUE, include = FALSE}
dm_perc_cat_hubs %>%
  filter(hub %in% c("Northwest", 
                    "California", 
                    "Southwest", 
...
Scherer and Karamanis put `year` in rows and `hub` (region) in columns. The `switch = "y"` argument moves the year label from the right side (where it appears by default) to the left. With this code in place, we can see the final plot coming together in Figure \@ref(fig:drought-viz-faceted). 

```{r results='asis'}
print_nostarch_file_name()
```

```{r ref.label = "drought-viz-faceted", fig.height = 8, echo = FALSE, fig.cap="The faceted version of the drought visualization. Space considerations require me to include only four regions, but you get the idea."}
dm_perc_cat_hubs %>%
  filter(hub %in% c("Northwest", 
                    "California", 
                    "Southwest", 
...
Let’s look at a few of the small polishes that Scherer and Karamanis made. The first is to apply a theme, as seen in Figure \@ref(fig:drought-viz-theme-light). They used `theme_light()`, which removes the default gray background and changes the font to Roboto.
...
```{r drought-viz-theme-tweaks, fig.height = 8, echo = TRUE, include = FALSE}
dm_perc_cat_hubs %>%
  filter(hub %in% c("Northwest", 
                    "California", 
                    "Southwest", 
...
They set some additional aesthetic properties specific to this geom: `xmin`, `xmax`, `ymin`, and `ymax`, which determine the boundaries of the rectangle it produces. The result is a gray background drawn behind each small multiple, as seen in Figure \@ref(fig:drought-viz-theme-tweaks).

```{r results='asis'}
print_nostarch_file_name()
```

```{r drought-viz-theme-tweaks, fig.height = 8, echo = FALSE, fig.cap="Figure 1-16	Faceted version of the drought visualization with gray backgrounds behind each small multiple"}
dm_perc_cat_hubs %>%
  filter(hub %in% c("Northwest", 
                    "California", 
                    "Southwest", 

...
Let's look at a few of the small polishes that Cédric and Georgios make. The first is to apply a theme. They use `theme_light()`, which removes the default gray background, changes the font to Roboto, and makes the default size of elements larger using the `base_size` argument, among other tweaks. 

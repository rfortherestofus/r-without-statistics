...
```{r out.width="100%", fig.cap="Abdoul Madjid's map of COVID in the United States in 2021"}
...
If you are making maps today in R, consider yourself lucky. Changes in recent years have made it much simpler to work with geospatial data, and to build maps with R. Before then, there were competing standards for geospatial data, and each standard required learning a different approach. Today, though, the simple features model (often abbreviated as sf) for working with vector geospatial data has become dominant. Be grateful for this, as simple features data is way easier to work with than were previous geospatial data models. 

Geospatial data is both very similar to data you're already used to working with in R and, in some ways, very different. To see the similarities and differences, let's look at how simple features data works. In this section, I'll just show you data and the maps we make with that data, providing the bare minimum you need to know in order to understand how simple features data works. The sections that follow will explain how we work wtih geospatial data. 

Let's begin by taking a look at some simple features geospatial data that represents the U.S. state of Wyoming. I've imported the data (using a package called `tigris` that I discuss in the conclusion of this chapter) and saved it as an object called `wyoming`. Here's what it looks like.
...
If you're curious about how to change projections when making maps of your own, fear not. I'll show you how to do so after we look at Abdoul Madjid's map. 
...
Second: the final product that Abdoul Madjid made is giant in order to fit all 365 maps. I've had to change sizes of a few elements below to make them fit in this book. I've kept the overall stucture of the code, but changed a few specific elements (for example, making text sizes smaller) so that everything fits.
...
The three lines of code that we used above to make our Wyoming maps come back again, with some adornments to improve the quality of our map. Below, we can see that we're using `geom_sf()` to plot our geospatial data. Madjid modifies a couple arguments, using `size = .05` to make the state borders less prominent and `color = "grey55"` to make the borders a medium grey color. Then, because he is making one map for each day, he uses the `facet_wrap()` function. The `vars(fancy_date)` code specifies that the `fancy_date` variable should be used to do the facetting (in other words, make one map for each unique observation of `fancy_date`) and `strip.position = "bottom"` moves the labels "Jan. 01", "Jan. 02", and so on to the bottom of the maps. 
...
The resulting map can be seen in TODO add figure number below. 
...
Our in-progress map can be seen below.
...
We can see the result of these tweaks below.
...
Next, Madjid made tweaks to the title and caption. Below, we added the title and caption using this `labs()` function. Then, we see how Madjid used the `theme()` function to tweak these elements as well as the label for each map ("Jan. 01", "Jan. 02", and so on). He makes all text use Times New Roman font and makes it a nearly black color using the hex code #111111. This sets defaults for all text elements, but Madjid then also makes tweaks to individual elements as well. He sets the size of the title to be 2.5 times that of other text elements (`size = rel(2.5)`), makes it bold (`face = "bold"`) and centered (`hjust = 0.5`), and adds 0.25 centtimer margins on the top and bottom (`hjust = 0.5, margin = margin(t = .25, b = .25, unit = "cm")`). He makes the same tweaks to the caption, with the exception of adjusting the font size. And finally, Abdoul Madjid adjusts the size and bolding of the individual map labels using `strip.text`.
...
Our map is looking good! Just a few tweaks left to get it close to Abdoul Madjid's original. By default, maps made with the `geom_sf()` function have text and lines to show longitude and latitude. These aren't not necessary so Madjid removed them using the `element_blank()` function on `panel.grid` and `axis.text`.
...
The result is a much nicer-looking set of maps. 
...
And there we have it: our recreation of Abdoul Madjid's COVID-19 map.
...
If this chapter has inspired you to want to make your own maps in R, fantastic! As we've seen from Abdoul Madjid's work, you can use R both to gather and analyze geospatial data as well as to plot it. R is a swiss army knife, with a tool you can pull out to do pretty much anything you can imagine. 

### Tutorial Stuff

In the first part of this chapter, I gave you a quick overview of how geospatial data works. You may now be wondering: ok, great, but how do I actually make my own maps? Let's talk first about where you can find geospatial data. 

#### Where to Get Data


##### How to read in raw geospatial data


First, a reminder: we're exclusively discussing vector data in the simple features format. While there are other types of geospatial data, simple features data is the simplest to work with and, as a result, increasingly dominant. 

With that said, there are two ways to access geospatial data. The first is to import raw data. Geospatial data can take a number of different formats. While ESRL shapefiles (with the .shp extension) are the most common, there is also GeoJSON (.geojson), KML (.kml), and others (chapter 8 of *Geocomputation with R* by Robin Lovelace, Jakub Nowosad, and Jannes Muenchow discusses the range of formats). The good news for us as users is that there is a single function that can read in pretty much any type of geospatial data. That function is `read_sf()` from the `sf` package. Let's show an example of how it works. 
...
To prove that this is geospatial data like that we've seen throughout this chapter, let's make a map with it. I'll use the same three lines from before. 
...
And there we have it: a map of all 50 states.
...
##### Packages to access APIs

Working with raw geosaptial data in this way is sometimes necessary. But oftentimes it's not. That's because there are R packages that provide functions to access geospatial data. Abdoul Madjid used the `usa_sf()` function from the `albersusa` package to bring in his geospatial data. Another package for accessing geospatial data related to the United States is called `tigris`. This package has a number of well-named functions for bringing in different types of data. One of these is the `states()` function. We can load the `tigris` package and run the `states()` function. We use the `cb = TRUE` argument to not use the most detailed shapefile (which can be quite large, depending on your data) and set the resolution to "20m" instead. We then save the result as `states_tigris`.
...
And we can plot our data on a map.
...
Once you have access to geospatial data and are ready to map it, you need to think about which projection to use. If you're looking for a simple answer to the question "which projection should I use?" you're likely to be disappointed. As Robin Lovelace, Jakub Nowosad, and Jannes Muenchow put it in their book *Geocomputation with R*, "the question of *which* CRS [to use] is tricky, and there is rarely a 'right' answer."  They continue: "properties of the Earth’s surface are distorted in this process, such as area, direction, distance, and shape. A projected coordinate system can preserve only one or two of those properties." 

If you feel overwhelmed with choosing a projection, there is a package to help. The `crsuggest` package, made by Kyle Walker (who is also the developer of `tigris`), can give you ideas for a projection to use. The `suggest_top_crs()` function will return a CRS that is well-suited for your data. Let's load `crsuggest` and try it out on our `africa_countries` data. 
...
When we run this code, we see Africa mapped with a different projection. 
...
And we end up with a map like this: 
...
This is a simple example of the type of data wrangling and analysis you can do on simple features data. The larger lesson to take here is this: all of the skills you've worked hard to develop to work with data in R will also serve you well when working with geospatial data. 

### Conceptual Stuff: Why R is Best Tool for Maps

This chapter has been a short if exhausting romp through the world of map-making in R (don't know about you, but I'm tired!). We've discussed the basics needed to understand geospatial data, seen how Abdoul Madjid applied this knowledge and principles of high-quality data visualization to make his map, learned how to get your own geospatial data and project it appropriately to make your own maps. 

Over the years, I've come to believe that R is not just a tool for making maps, but the best tool for making maps. It's way more flexible than Excel and way less expensive than ArcGIS. But R as a map-making tool isn't just good because it's free. It's good because you can use the general skills you've developed working with traditional data frames to your work with geospatial data. And the same ggplot code that makes your graphs shine will also help you make maps look great. 

That's what Abdoul Madjid did when making his map of COVID in the United States in 2021. Madjid is not a GIS expert, but that's kind of the point. Combine an understanding of the basics of geospatial data, fundamental R skills, and knowledge of data visualization principles and he made a beautiful map. If Abdoul Madjid can do this, so can you.

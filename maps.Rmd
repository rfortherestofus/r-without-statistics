...
First, we import COVID data. This data comes directly from the *New York Times*, which publishes daily case rates by state as a CSV on their GitHub account. 

```{r echo = TRUE}
covid_data <- read_csv("https://raw.githubusercontent.com/nytimes/covid-19-data/master/us-states.csv")
```

If we take a look at this data we can see the arrival of the first COVID cases in the United States. All of the variables are self-explanatory except for `fips`, which is the Federal Information Processing Standards codes used to represent states. 
...
covid_evolution_plot <- usa_states_geom %>%
  ...
  theme(
    plot.title = element_text(
      color = "#111111", family = font_family, size = rel(5), face = "bold", margin = margin(t = .5, b = .5, unit = "cm"),
      hjust = 0.5
    ),

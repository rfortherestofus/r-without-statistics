...
In this chapter, we will begin by diving into geospatial data, giving you the minimum you need to know in order to make maps in R. We'll then walk through Abdoul Madjid's code, looking at the choices he made that resulted in this high-quality map. I then offer some guidance on where to find geospatial data and how to work with it to make your own maps. The chapter concludes with some thoughts about why R is such a good tool for making maps.
...
knitr::include_graphics(here::here("assets/sf-data.png"))
```
...
Let's discuss each of these differences, starting with metadata. Above the main output, we see a section that starts with the text "Simple feature collection with 1 feature and 1 field." The feature here is the row and the field is the variable (`NAME`) that contains non-spatial data (the `geometry` column, which contains geospatial data, will be discussed below). This line, and the lines that follow, are metadata about the geospatial data in the `wyoming` object. Let's look at each piece of the metadata.
...
```{r us-map-albers-projection-code, echo = TRUE, eval = FALSE}
usa_states_geom %>%
  ggplot() +
  geom_sf()
```
...
```{r us-map-albers-projection, ref.label = "us-map-albers-projection-code", fig.cap = "A map of the United States using the Albers equal-area conic convenience projection"}
```
...
```{r us-map-wgs84-projection-code, echo = TRUE, eval = FALSE}
usa_states_wgs84 %>%
  ggplot() +
  geom_sf()
```
...
```{r us-map-wgs84-projection, ref.label = "us-map-wgs84-projection-code", fig.cap = "A map of the United States using the WGS84 projection"}
```
...
```{r map-with-rocket-palette-code, echo = TRUE, eval = FALSE}
usa_states_geom_covid_six_days %>%
  ggplot() +
  geom_sf(
    aes(fill = incidence_rate),
...
```{r map-with-rocket-palette, ref.label = "map-with-rocket-palette-code", fig.cap = "The COVID map with the rocket palette added"}
```
...
```{r map-with-more-legend-tweaks-code, echo = TRUE, eval = FALSE}
map_with_legend_tweaks +
  theme_minimal() +
  theme(
    legend.position = "top",
...
```{r map-with-more-legend-tweaks, ref.label = "map-with-more-legend-tweaks-code", fig.cap = "The COVID map with additional tweaks to the legend"}
```
...
```{r map-with-text-tweaks, ref.label = "map-with-text-tweaks-code", fig.cap = "The COVID map with tweaks to the title, captions, and strip text"}
```
...
```{r map-with-grid-tweaks-code, echo = TRUE, eval = FALSE}
...
```{r map-with-grid-tweaks, ref.label = "map-with-grid-tweaks-code", fig.cap = "The COVID map without the grid lines and axis text"}
```
...
```{r map-with-background-tweaks-code, echo = TRUE, eval = FALSE}
...
```{r map-with-background-tweaks, ref.label = "map-with-background-tweaks-code", fig.cap = "The COVID map with tweaks to the margins and the background color"}
```
...
```{r eojson-us-map-code, echo = TRUE, eval = FALSE}
us_states %>% 
  ggplot() +
  geom_sf() 
```
...
```{r geojson-us-map, ref.label = "eojson-us-map-code", fig.cap = "A map of the United States made with GeoJSON data"}
```
...
Working with raw geosaptial data in this way is sometimes necessary. But not always. That's because there are R packages that provide functions to access geospatial data. Abdoul Madjid used the `usa_sf()` function from the `albersusa` package to bring in his geospatial data. Another package for accessing geospatial data related to the United States is called `tigris`. This package has a number of well-named functions for bringing in different types of data. One of these is the `states()` function. We can load the `tigris` package and run the `states()` function. Let's use the `cb = TRUE` argument to not use the most detailed shapefile and set the resolution to a more manageable "20m" (without these changes, the shapefile we'd get would be large and slow to work with). We also set `progress_bar = FALSE` in order to not see messages that `tigris` shares while it loads data. We then save the result as `states_tigris`.
...
```{r tigris-us-map-code, echo = TRUE, eval = FALSE}
states_tigris %>% 
  shift_geometry() %>%
  ggplot() +
  geom_sf()
...
```{r tigris-us-map, ref.label = "tigris-us-map-code", fig.cap = "A map of the United States made with data from the tigris package"}
```

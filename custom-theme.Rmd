...
If we now compare our plot to a 2019 chart that Nassos and Clara made for an article on the carbon footprint of various food items, we can see how similar it is. Figure \@ref(fig: bbc-beef-chart) has the same large text, no axis titles or grid lines, legend on the top, and more. 
...
When Nassos Stylianou and Clara Guibourg started thinking of developing a custom theme for the BBC, they had one goal in mind: could they write code that would enable people to create graphs in R that could go straight onto the BBC website? And, wouldn't you know, they succeeded! The creation of the `bbc_style()` function within the `bbplot` package allowed them to make plots that had a consistent look-and-feel, followed BBC standards, and, most importantly, did not need the intervention of a designer in order to be published. 

Many of the principles of high-quality data visualization that we discussed in \@ref(data-viz-chapter) can be seen in this custom theme. In particular, the stripping out of extraneous elements (axis titles and grid lines, for instance) helps keep the focus on the data itself. And by creating a custom theme that only requires the user to add a single line (`bbc_style()`) to their ggplot code, it became simple to get others on board. 

The development of the `bbplot` package would lead to significant changes at the BBC. It inspired the data journalists who created it to use ggplot more than before. Knowing that they had the flexibility of ggplot at their fingertips gave them license to explore. And knowing that they did not have to work with a designer to create production-ready graphics empowered them to make more graphics than they had previously. 

In addition to the `bbc_style()` function, the `bbplot` package also provides another funtion (`finalise_plot()`) that adds a source at the bottom of the chart (recall how the `bbc_style()` function removed the caption), adds the BBC logo in the footer and gives height, width, and file name options for saving the plot. These two functions combined allowed Nassos, Clara, and others to achieve their holy grail: creating production-ready graphs that could go straight from R to the BBC website. 

The impact of `bbplot` would also come to be seen outside of the small team of data journalists that brought it to life. Others at the BBC saw how the data journalism team was now able to produce production-ready graphs and they wanted to do the same. This led the data journalism team to set up R trainings for their colleagues and to develop a "cookbook" that provied examples of how to make various types of charts. These two resources led to a large increase in R users at the BBC, and a large increase in the number of ggplot-generated charts on the BBC website.

TODO: Add quote to previous graf

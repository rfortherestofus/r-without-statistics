...
In mid-2020, staff at the Urban Institute were tasked with developing State Fiscal Briefs. The Urban Institute, a well-respected Washington D.C.-based think tank, regularly produces economic and social policy research and these State Fiscal Briefs were designed to provide an overview of the fiscal situation of all U.S. states and the District of Columbia. At a time when COVID-19 had brought much economic activity to a halt, these reports would show just how bad the situation had become. 
...
In this chapter, I'll begin by explaining what parameterized reporting is. We'll then work through a simplified version of the code that the Urban Institute used to demonstrate parameterized reporting in action. We'll conclude with some reflections on the value of parameterized reporting. Throughout the chapter, you'll hear insights about parameterized reporting from my interview with Sayed, Mucciolo, and Williams. 
...
We've now seen how setting a parameter in the YAML gives us the ability to dynamically generate text and charts in our knitted report. But we've only generated one report so far. How could we now create all 51 reports?

Your first thought might be to manually update the YAML. Indeed you could go in, change Alabama to Alaska, and knit again in order to get a report for that state. You could do this same thing for all states, but it would be tedious, and we're trying to avoid tedium. 

### Creating an R Script File to Render Multiple Reports {-}

At this point, we're going to move out of R Markdown and into an R script file. The first thing to know is that, while you've seen how to knit an R Markdown document using the Knit button, you can do the same thing with code. There is a package called `rmarkdown` and a function within it called `render()`. If I load the `rmarkdown` package and then use the `render()` function, as in the code below, the resulting HTML document will show up.
...
If we run this code, our HTML document will be called `urban-covid-budget-report.html`. We can change its name by using the `output_file` argument. 
...
The next step is to create a tibble with information needed to render all 51 reports. We do this by creating an object called `reports`. This object has multiple arguments that we can pass to the `render()` function. Above, we used `render()` with only the `input` argument, but you can also pass it arguments such as `output_file` and `params`. The code below generates a tibble with 51 rows and three variables. 
...
Once we've created the `reports` tibble, we're ready for our third and final step: creating all of our reports. The code that generates all of the reports is only two lines. We use the `pwalk()` function, which comes from the `purrr` package. This function has two arguments: 1) a data frame (`reports` in our case), and 2) a function to run on each row of this data frame (`render` though not that you do not include open and close parentheses typically seen with functions).
...
When we run this code, it runs the `render()` function for each row in `reports`, each time passing in the values for `input`, `output_file`, and `params`. It is the equivalent of running this code (though I've only listed two of the 51 iterations of `render()` we'd need to use):
...
Being able to tell R to run the Alabama row to generate its report, then the Alaska row, and then all other states is a huge time-saver. If I run the `pwalk(reports, render)` code, I quickly see 51 HTML documents appear. And each one will be a report for that state, complete with a customized graph and accompanying text. 

## Best Practices for Working with Parameterized Reporting {-}

Working with parameterized reporting is incredibly powerful. It can also present some challenges. I asked team at the Urban Institute for their thoughts on best practices for parameterized reporting. Their feedback focused on one topic: considering outliers. 
...
Another best practice that the Urban Institute team recommends is to manually generate reports with the shortest and longest value of the parameter you're working with (in the State Fiscal Briefs, this would be Iowa and District of Columbia). Making and reviewing these reports manually allows you to see places where the length of the text may cause unexpected results. Titles in charts can be cut off, text may run onto multiple lines, messing up page breaks in PDF reports, and so on. A few minutes of manual work early on can make the automated process of generating multiple reports much smoother in the end.

## In Conclusion: Parameterized Reporting TODO

In this chapter, we've worked through the example of the State Fiscal Briefs that the Urban Institute team made using parameterized reporting. Automating the production of 51 reports was huge time-saver, though at this scale, it would still be feasible to make these reports by hand. 

Near the end of our interview, Aaron Williams gave me an example of another project the Urban Institute team works on. This project involves making county-level reports. There are over 3,000 counties in the United States. Making these reports by hand is simply not feasible. With parameterized reporting, though, it is the same exact process to make 3,000 reports as it is to make 51. Parameterized reporting isn't just a fun tool. It makes new options for reporting possible. 

Beyond opening up new possibilities, parameterized reporting also makes your work more accurate. If the Urban Institute were to make these reports using, say, Excel and Word, there would be a ton of copying and pasting. Humans are fallible, and mistakes occur no matter how hard we try to avoid them. Computers do not make copy-paste errors. Letting them handle the tedious work of making multiple reports reduces the chance of error significantly.
...
As with many things with R, it can feel like a heavy lift to produce reports using parameterized reporting. Initially, it is. You have to make sure that your code works not just for, say, one state, but for all 51. There can be challenges with outliers. But once you have your R Markdown document and accompanying R script file for rendering, it is straightforward to produce multiple reports at once. It may be more work in the beginning, but it is far, far less work in the end.

...
```{r penguins-report-slide-1, out.width="100%", fig.cap="The first slide of the presentation"}
...
```{r penguins-report-slide-2, out.width="100%", fig.cap="The second slide of the presentation"}
...
The syntax we just used to center the entire slide can also enable us to add a background image. Let’s create a new slide, use the classes `center` and `middle` to horizontally and vertically align the content, and add a background image by surrounding the path to the image with `url()`.
...
```{r penguins-report, out.width="100%", fig.cap="Figure 8-6	The title slide with changes to the text to make it more visible"}

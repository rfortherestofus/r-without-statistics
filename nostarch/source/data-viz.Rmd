...
```{r final-viz, fig.height = 8, fig.cap="Section of the final drought visualization"}
dm_perc_cat_hubs %>%
...
```{r cluttered-viz, fig.height = 8, fig.cap="Cluttered version of the drought visualization"}
dm_perc_cat_hubs %>%
  filter(hub %in% c("Northwest", 
                    "California", 
                    "Southwest", 
...
```{r viz-sw-2003, fig.height = 4, fig.cap="Drought visualization for Southwest in 2003"}
dm_perc_cat_hubs %>%
...
```{r excel-chart-chooser, out.width="100%", fig.cap="Excel chart chooser menu"}
...
```{r blank-ggplot, echo = TRUE, fig.cap="A blank chart"}
ggplot(
  data = gapminder_10_rows,
  mapping = aes(
    x = year,
...
Let's say we change our mind and want to make a line chart instead. Well, all we have to do is replace `geom_point()` with `geom_line()` and we get \@ref(fig:gapminder-line). 
...
```{r gapminder-bar-colors, echo = TRUE, fig.cap="The same chart, now will added colors"}
ggplot(
  data = gapminder_10_rows,
  mapping = aes(
    x = year,
...
```{r drought-viz-legend-tweaks, fig.height = 8, fig.cap="Drought visualization with changes made to the legend text"}
dm_perc_cat_hubs %>%
  filter(hub %in% c("Northwest", 
                    "California", 
                    "Southwest", 

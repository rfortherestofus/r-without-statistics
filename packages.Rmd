...
1. Create a `theme_dk.Rd` file in the `man` directory. This file is the documentation of our `theme_dk()` function in the very specific format that R packages need. You're welcome to look at it, but can't change it since it is read only. The workflow outlined above with Roxygen is a much easier way to generate `.Rd` files than doing so by hand.
2. 

...
To create documentation for `theme_dk()`, place your cursor anywhere in the function. Then, go to the Code menu and select Insert Roxygen Skeleton (Roxygen is a way to create documentation of functions in R packages using a package called `roxygen2`), as seen in Figure \@ref(fig:TODO) below.
...
This text is the skeleton of what is needed to create documentation of our function. The most notable feature is the `#'` at the start of each line. These are special comments that are used for Roxygen. We can now go in and edit the Roxygen text to create our documentation. Starting at the top, I'll replace Title with a sentence that describes my function.

```
#' David Keyes's custom ggplot theme 
```

Next, you can see lines with the text `@param`. Roxygen automatically looks at the arguments in our function and creates one line for each. On each line, we want to describe what the argument does. I can do this as follows.
...
The last section is `@examples`. This is where you can give examples of code that users can run to learn how the function works. Doing this introduces some complexities so I'm going to skip it, but Chapter 17 of the second edition of Hadley Wickham and Jenny Bryan's book *R Packages* is a great place to learn about adding examples.
...
**Create a `theme_dk.Rd` file in the `man` directory**. This file is the documentation of our `theme_dk()` function in the very specific format that R packages need. You're welcome to look at it, but can't change it since it is read only. The workflow outlined above with Roxygen is a much easier way to generate `.Rd` files than doing so by hand.
...
The DESCRIPTION file lists meta data about the package. We can make a few changes to change the Title, add an Author and Maintainer, and add a Description. The resulting DESCRIPTION file might look something like this:
...
Having developed our package, we're now ready to distribute it.

### Installing Package

When you're developing your own package, installing it for your own use is relatively straightforward. Simply run `devtools::install()` in your console and the package will be ready for you to use in any project. Once I install the `dk` package, for example, I can just add the line `library(dk)` in any R code I'm writing and I'll then be able to use the `theme_dk()` function.

Of course, if you're developing a package, you're likely doing it not just for your self, but for others. There are many ways you can make your package accessible to others, but the most common is to use GitHub. The details of how to use GitHub are beyond what we can cover here, but the book *Happy Git and GitHub for the useR* by Jenny Bryan is a great place to get started. 

I've pushed the `dk` package to GitHub and you can find it at https://github.com/dgkeyes/dk. If you or anyone else wants to install it, you can run the code `remotes::install_github("dgkeyes/dk")`. That's it! 

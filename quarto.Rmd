...
As you've seen throughout this chapter, we can do everything we did with R Markdown in Quarto as well. Quarto also has some nice features that differentiate it from R Markdown. 

One of these new features is being able to easily publish any type of HTML document (reports, slides, websites, and more) to a free service called Quarto Pub. Especially if you are not a GitHub user, this is a great option to get your materials online. To show you how Quarto Pub works, let me publish the website we just made to it. To get started, look for the Terminal tab on the bottom left panel of RStudio. Click it and at the prompt enter the text `quarto publish`. Doing so will bring up a list of ways you can publish your website. See Figure \@ref(fig:TODO) below.
